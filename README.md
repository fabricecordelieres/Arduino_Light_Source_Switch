...
The electronic design is pretty simple. A [Fritzing scheme](../tree/master/Fritzing/Arduino-Light_Source_Switch.fzz) is enclosed in this repository:
...
### 3D printed parts
A [FreeCAD file](../tree/master/FreeCAD/Arduino-Light_Source_Switch.FCStd) and [STL files](../tree/master/STL/) are provided as part of this repository, allowing to print the enclosure for the circuit board, the support for the servo and the slider.
...
### Programming
A [Arduino script](../tree/master/Arduino/Arduino-Light_Source_Switch.ino) is available form the repository. It basically handles the input from two sources:
...
* *Right/left positions are not correct:* in such cases, adapt the [Arduino script](../tree/master/Arduino/Arduino-Light Source Switch.ino) by tweaking the ```rightPosition``` and ```leftPosition``` variables' values;
* *Pressing once the button will make the slider move quickly right-left-right or left-right-right:* you should adapt the refractory period in the [Arduino script](../tree/master/Arduino/Arduino-Light_Source_Switch.ino) by modifying the ```timeout``` variable' value.
